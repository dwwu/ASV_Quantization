...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  elif model_architecture == 'conv_bn':
    return create_conv_bn_model(fingerprint_input, model_settings, is_training)
...
def create_conv_bn_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
  ...
  first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, 1, 1, 1],
                            'SAME') + first_bias
  first_bn = tf.layers.batch_normalization(first_conv, training=is_training,
          name='first_bn')
  ...
  second_conv = tf.nn.conv2d(max_pool, second_weights, [1, 1, 1, 1],
                             'SAME') + second_bias
  second_bn = tf.layers.batch_normalization(second_conv, training=is_training,
          name='second_bn')
...
# def create_tdnn_model(fingerprint_input, model_settings, is_training):
  # input_frequency_size = model_settings['fingerprint_width']
  # input_time_size = model_settings['spectrogram_length']
  # fingerprint_4d = tf.reshape(fingerprint_input,
                              # [-1, input_time_size, input_frequency_size, 1])
