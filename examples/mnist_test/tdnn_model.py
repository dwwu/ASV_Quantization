...
def tdnn_config(model_size):
    if model_size == 'S':
        ...
        print("invalid model_size: {}".format(model_size))
...
def make_quant_tdnn_model(config, n_labels, n_frames=None):
    """
        dimension = (N, W, H, C) = (batch_size, T, 1, feat) for channel_last
        :return:
    """
    ...
    model.add(tf.keras.layers.BatchNormalization(axis=-1, fused=False))
    ...
    model.add(tf.keras.layers.Dense(fc_dim, activation='relu', use_bias=True,
        name='xvector'))
    model.add(tf.keras.layers.BatchNormalization(axis=-1, fused=False))
...
def make_quant_tdnn_model_mnist(config, n_labels, n_frames=None):
    """
        dimension = (N, W, H, C) = (batch_size, T, 1, feat) for channel_last
        :return:
    """
